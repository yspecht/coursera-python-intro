...
def problem3_4(mon, day, year):
    mnths = {"January":1, "Febraury":2, "March":3, "April":4, "May":5, "June":6, "July":7, "August":8, "September":9, "October":10, "November":11, "December":12}
    mon = mnths[mon]
...
def problem3_7(csv_pricefile, flower):
    ...
    flwr = str(flower)
    price = open(csv_pricefile)
    for row in csv.reader(price):
        if row[0] == flwr:
            print(row[1])
            break
